...
def print_surface_format(surface_format):
    profile_name = 'core' if surface_format.profile(
    ) == QSurfaceFormat.CoreProfile else 'compatibility'
...
class RenderWindow(QWindow):
    ...
    def init_gl(self):
        ...
        if (fmt.renderableType() == QSurfaceFormat.OpenGL and fmt.majorVersion() == 3
            ...
            use_new_style_shader = not fmt.testOption(
        ...
        if not self.program.addShaderFromSourceCode(QOpenGLShader.Vertex, vertex_shader):
            ...
            raise Exception(
                "Vertex shader could not be added: {log} ({vertexShader})")
        if not self.program.addShaderFromSourceCode(QOpenGLShader.Fragment, fragment_shader):
            log = self.program.log()
            raise Exception(
                f"Fragment shader could not be added: {log} ({fragment_shader})")
        ...
        self.vbo.allocate(VoidPtr(self._vertices_data),
    ...
    def setup_vertex_attribs(self):
        ...
        self.program.setAttributeBuffer(
            self._col_attr, GL.GL_FLOAT, 4 * vertices.size, 3)
